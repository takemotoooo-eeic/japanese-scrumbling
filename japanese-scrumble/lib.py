...
def get_surprisal_dataframe_id(phenomenon, model, names_size, grammar_version, sentence_file, grammar_idx):
...
def _pad_sentence(s: SentenceSurprisal, pad_size: int) -> SentenceSurprisal:
    ...
    pad_tokens = tuple(Token(text=PADDING, surprisal=np.nan, idx=i) for i in range(pad_start_idx, pad_start_idx + pad_size))
...
def get_surprisals_for_strings(strings: tuple[str, ...], model: str) -> SentenceSurprisal:
    return get_surprisals_per_model(strings, (model,))[model]


def get_surprisals_per_model(sentences: tuple[str, ...], models: tuple[str, ...]) -> dict[str, tuple[SentenceSurprisal, ...]]:
...
def _get_running_container_id(model: str) -> Optional[str]:
    ...
    logger.debug(f"Container for model {model_base} checkpoint {checkpoint} already running: {container_id}")
    return container_id
...
def _start_container(model: str) -> str:
    ...
    if model_base == _JRNN:
        post_command = (
            "cp /tmp/pos-mount/modified_external_sources/lm-zoo/JRNN/bin/get_surprisals_concurrent /opt/bin/get_surprisals"
        )
    ...
    if post_command:
        ...
        if process.returncode != 0:
            ...
            raise RuntimeError(f"Error running Docker command on {short_id}: {process.stderr.decode('utf-8')}")
...
def tokens_to_sentence(token_strings: tuple[str, ...]) -> Sentence:
    ...
    critical_region_idxs, tokens_without_markers = _extract_critical_region_idxs(tokens_without_pads)

    preprocessed_tokens = []
    for t, token_str in enumerate(tokens_without_markers):
        preprocessed_tokens.append(Token(text=token_str, critical_region=t in critical_region_idxs, idx=t))
...
def _get_surprisals_from_openai(sentences: tuple[Sentence, ...], engine: str) -> tuple[SentenceSurprisal, ...]:
    import openai
    ...
    for c, choice in enumerate(choices):
        ...
        for t, token_text in enumerate(tokens):
            ...
            surprisal_tokens.append(Token(text=token_text.strip(), surprisal=surprisal, idx=current_token_idx))
...
def _reintroduce_padding_tokens(
    ...
    for padding_idx in surprisal.original_sentence.padding_idxs:
        insertion_points = tuple(filter(lambda t: t[1].idx == padding_idx, enumerate(new_tokens)))
        insertion_idx, _ = insertion_points[-1]
        new_tokens = (
            new_tokens[:insertion_idx] + [Token(text=PADDING, surprisal=np.nan, idx=padding_idx)] + new_tokens[insertion_idx:]
        )
...
def remove_model_cache(model):
    ...
    response = input(f"This will delete {num_keys} entries from cache. Are you sure? Y/n\n")
    if response != "Y":
        return
    total_deleted = 0
...
def store_in_cache(model: str, sentence_surprisal: SentenceSurprisal):
    ...
    original_tokens = tuple(x.text for x in sentence_surprisal.original_sentence.preprocessed_tokens)
    if surprisal_tokens != original_tokens:
        logger.warning(f"Cache token mismatch:\n\t{original_tokens}\n\t{surprisal_tokens}")

    _SURPRISAL_CACHE[_cache_key(sentence_surprisal.original_sentence, model)] = sentence_surprisal.to_json()


def get_surprisal_from_cache(sentence: Sentence, model: str) -> Optional[SentenceSurprisal]:
...
def _tokenize_for_yedetore_models(sentence: Sentence) -> tuple[Token, ...]:
    ...
    for original_token_idx, original_token in enumerate(sentence.preprocessed_tokens):
        subwords = childes_vocab.split_possesives_and_contractions(original_token.text).split(" ")
...
def _get_surprisal_tokens_from_yedetore_model(
    ...
    next_word_probabs = torch.take_along_dim(probabs[:, :-1], input_word_ids[:, 1:].unsqueeze(-1), dim=-1)
    ...
    for i, token in enumerate(input_tokens):
        if token.text not in dictionary.word2idx:
            ...
            text = token.text
        surprisal_tokens.append(Token(text=text, surprisal=surprisals[0, i, 0].item(), idx=token.idx))
...
def _load_childes_transformer(model_name, device):
    ...
    m = yedetore_model.TransformerModel(ntoken=17094, ninp=800, nhead=4, nhid=800, nlayers=4)
    m.load_state_dict(m_state_dict)
    return m


def _get_surprisals_from_retrained_transformer(sentences: tuple[Sentence, ...], model_name: str):
    ...
    from modified_external_sources.colorlessgreenRNNs.src.language_models import dictionary_corpus as grnn_dictionary_corpus
...
def _get_surprisals_from_childes_model(sentences: tuple[Sentence, ...], model: str) -> tuple[SentenceSurprisal, ...]:
    device = utils.get_device()
...
def _get_bert_masked_surprisal(sentence: Sentence, bert_pipeline, bert_model: str) -> SentenceSurprisal:
    critical_tokens = [(i, x) for i, x in enumerate(sentence.preprocessed_tokens) if x.critical_region]
    assert len(critical_tokens) == 1, f"BERT surprisals can only be measured at single masked token"
    ...
    masked_str = " ".join([token.text if not token.critical_region else "[MASK]" for token in sentence.preprocessed_tokens])
    ...
    sentence_surprisal = SentenceSurprisal(tokens=tuple(tokens), model=bert_model, original_sentence=sentence)
    return sentence_surprisal


def _get_surprisals_from_bert(sentences: tuple[Sentence, ...], bert_model: str) -> tuple[SentenceSurprisal, ...]:
    bert_pipeline = transformers.pipeline("fill-mask", model=bert_model)
    return tuple(_get_bert_masked_surprisal(sentence, bert_pipeline, bert_model) for sentence in sentences)


def get_surprisals(sentences: tuple[Sentence, ...], model: str) -> tuple[SentenceSurprisal, ...]:
    (
        cached_sentences,
        non_cached_sentences_to_idx,
    ...
    if non_cached_sentences:
        ...
        if model.startswith(f"{GPT3}__"):
            ...
            if os.getenv("POS_LM_DEV_MACHINE"):
                logger.warning(f"Dev machine, using cheaper GPT-3 model {_GPT3_CHEAP_ENGINE}.")
                engine = _GPT3_CHEAP_ENGINE

            surprisals = _get_surprisals_from_openai(non_cached_sentences, engine=engine)
        elif model.startswith("retrained__"):
            surprisals = _get_surprisals_from_retrained_transformer(non_cached_sentences, model)
            ...
            surprisals = _get_surprisals_from_bert(non_cached_sentences, bert_model=model)
        else:
        ...
        for sentence_surprisal in surprisals:
            results_from_live_model[non_cached_sentences_to_idx[sentence_surprisal.original_sentence]] = sentence_surprisal
...
def load_grammars(names_size, retraining_corpus=False) -> dict[str, dict[str, tuple[nltk.CFG, ...]]]:
    ...
    for p, phenomenon in enumerate(config["PHENOMENA"]):
        ...
        for g, grammar_str in enumerate(phenomenon_grammars):
            for condition, s_derivation_str in base_s_derivations.items():
                full_grammar_str = s_derivation_str + "\n" + base_grammar_str + "\n" + names_grammar_str + "\n" + grammar_str
...
def get_lexical_choices(output_tokens: tuple[str, ...], grammar: nltk.CFG) -> frozenset[tuple[str, str]]:
...
def iterate_phenomenon_sentences(phenomenon, names_size, conditions=None, retraining_corpus=False):
    phenomenon_to_grammars = load_grammars(names_size, retraining_corpus=retraining_corpus)
